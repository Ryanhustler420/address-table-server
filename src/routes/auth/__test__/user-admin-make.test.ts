...
it("return error if try to make admin a existing user but hit wrong url", async () => {
  const { cookie } = await register();
  const response = await request(app)
    .post("/api/auth/make-admin2")
    .set("Cookie", cookie)
    .send({
      email: email,
      code: "12345",
...
it("return error if try to make admin a existing user with correct url hit but wrong method", async () => {
  const { cookie } = await register();
  const response = await request(app)
    .get("/api/auth/make-admin")
    .set("Cookie", cookie)
    .send({
      email: email,
      code: "12345",
