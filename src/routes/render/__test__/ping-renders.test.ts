...
it("Success", async () => {
  ...
  (axios.get as jest.Mock).mockResolvedValueOnce({ data: { ok: 1 } });

  const ping = await request(app).post("/api/render/ping").expect(200);

  expect(ping.body).toHaveProperty("ack");
  expect(ping.body.ack).toContain("Ping Pong");
});
